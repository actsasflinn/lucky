...
class TextHelpers::Index < LuckyWeb::Action
  action { render_text "foo" }
end

class TextHelpers::Create < LuckyWeb::Action
  action { render_text "foo" }
end
...
describe LuckyWeb::TextHelpers do
  Spec.before_each do
    view.reset_cycles
  end
  ...
  it "truncates with options hash" do
  ...
  it "highlights pending" do
    view.highlight("   ", "blank text is returned verbatim").should eq "   "
  end

  it "highlights returning blank string for nil" do
  ...
  describe LuckyWeb::TextHelpers::Cycle do
    it "cycles" do
      value = LuckyWeb::TextHelpers::Cycle.new("one", 2, "3")
